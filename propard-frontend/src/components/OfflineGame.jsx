...
export default function OfflineGame({ onRetry }) {
  ...
  const saveHighScore = value => {
    try {
      ...
      // localStorage peut être indisponible dans certains contextes privés.
    }
  };

  const createStars = (width, height) => {
    const count = Math.max(45, Math.floor((width * height) / 11000));
  ...
  const resizeCanvas = () => {
    ...
    if (!game.player.x) {
      ...
      game.player.x = clamp(game.player.x, 30, width - 30);
      game.player.targetX = clamp(game.player.targetX, 30, width - 30);
    }
  ...
  const spawnObstacle = () => {
    ...
    const difficulty = Math.min(game.score / 50, 1);

    const width = random(72, 138);
    const height = random(42, 72);

    const obstacle = {
      x: random(width / 2 + 12, game.width - width / 2 - 12),
      y: -height - random(20, 100),
      width,
      height,
      rotation: random(-0.12, 0.12),
      rotationSpeed: random(-0.0015, 0.0015),
      phase: random(0, Math.PI * 2),
      phaseSpeed: random(0.0015, 0.003),
      speed: random(130, 175) + difficulty * 80,
      type:
        OBSTACLE_TYPES[
  ...
  const addParticles = (x, y, amount, speedMultiplier = 1) => {
    const game = gameRef.current;

    for (let i = 0; i < amount; i++) {
      const angle = random(0, Math.PI * 2);
      const speed = random(30, 130) * speedMultiplier;
      ...
      game.particles.push({
        ...
        life: random(0.3, 0.9),
        maxLife: random(0.3, 0.9),
        size: random(1.5, 4),
        gravity: random(10, 35)
      });
    }
  };
  ...
  const checkCollision = obstacle => {
    ...
    const halfW = obstacle.width * 0.43;
    const halfH = obstacle.height * 0.43;

    return (
      px > obstacle.x - halfW - 15 &&
      px < obstacle.x + halfW + 15 &&
      py > obstacle.y - halfH - 18 &&
      py < obstacle.y + halfH + 18
    );
  };
  ...
  const update = dt => {
    ...
    const speedMultiplier = 1 + Math.min(game.score / 70, 1.25);
    ...
    game.player.rotation = clamp(
      game.player.vx * -0.012,
      -0.35,
      0.35
    );
    ...
    for (const star of game.stars) {
      star.y += star.speed * 70 * dt * speedMultiplier;
    ...
    for (let i = game.obstacles.length - 1; i >= 0; i--) {
      ...
      obstacle.y += obstacle.speed * dt * speedMultiplier;
      obstacle.rotation += obstacle.rotationSpeed * dt * 60;
      obstacle.phase += obstacle.phaseSpeed * dt * 60;

      obstacle.renderX =
        obstacle.x + Math.sin(obstacle.phase) * 8;

      if (checkCollision({
        ...obstacle,
        x: obstacle.renderX
      })) {
        addParticles(game.player.x, game.player.y, 30, 1.6);
        endGame();
        return;
      }
  ...
  const drawBackground = ctx => {
    ...
    for (const star of game.stars) {
      const twinkle =
        0.65 +
        Math.sin(performance.now() * 0.002 + star.phase) *
          0.25;
    ...
    glow.addColorStop(
      0,
      colors.accent.replace(')', ', 0.08)').replace('rgb(', 'rgba(')
    );
  ...
  const drawObstacle = (ctx, obstacle) => {
    ...
    ctx.shadowBlur = 18;
    ctx.shadowColor = `${colors.accent}44`;
    ...
    const maxTextWidth = w - 52;

    let label = obstacle.type.label;
  ...
  const drawShip = ctx => {
    ...
    const flame = 16 + Math.sin(performance.now() * 0.025) * 5;
  ...
  const drawFloatingTexts = ctx => {
    ...
    for (const text of game.floatingTexts) {
      ctx.globalAlpha =
        Math.max(0, text.life / text.maxLife);
  ...
  const draw = () => {
    ...
    if (!canvas || !game.width || !game.height) {
      return;
    }
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 14,
                  cursor: 'pointer',
                  boxShadow: `0 8px 25px ${colors.accent}44`
                }}
              >
                Lancer le jeu
              </button>
          ...
          >
            <div
              style={{
                ...
                padding: '30px 24px',
                borderRadius: 20,
                background: `${colors.secondary}f2`,
                border: `1px solid ${colors.border}`,
                boxShadow:
